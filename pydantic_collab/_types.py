...
class CollabState:
    ...
    def record_execution(
        ...
        self.execution_history.append({
            ...
            'output': str(output_text)[:200] + '...' if
                len(str(output_text)) > 200 else str(output_text),
...
class CollabRunResult(Generic[OutputDataT]):
    ...
    _state: CollabState = field(
        repr=False, compare=False, default_factory=lambda: CollabState(query='')
    )
...
class CollabAgent:
    ...
    Can be agent names (str), AbstractAgent instances, or CollabAgent instances."""
    #name: str | None = None
    """Name of the agent, by default taken from the agent itself"""
...
class HandOffBase(BaseModel, Generic[T]):
    ...
    reasoning: str | None = Field(
        default=None,
        description='Optional explanation of why this action was taken. '
        'Useful for debugging and transparency.',
    )
    next_agent: str = Field(description='Name of the agent to route to.')
    query: T = Field(description='The query/message/response to send'
                                 ' to the next agent.')
...
class CollabSettings:
    ...
    output_restrictions: Literal['only_str', 'only_original', 'str_or_original'] = \
        'str_or_original'
    ...
    class HandOffOutput(HandOffBase[T]):
        ...
        if settings.include_handoff in static:
            ...
            include_previous_handoff: bool = Field(
                default=False,
                description='Whether to include the previous handoff history.'
            )

        if settings.include_tool_calls_with_callee in static:
            include_tool_calls_with_callee: ClassVar[bool] = (
                    settings.include_tool_calls_with_callee == 'force')
        else:
            include_tool_calls_with_callee: bool = Field(
                default=False,
                description='Whether to include tool calls with that agent if you had any '
        ...
        if settings.include_thinking in static:
            ...
            include_thinking: bool = Field(
                default=False, description='Whether to include thinking parts from the '
